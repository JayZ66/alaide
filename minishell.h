...
// Var. env.
char			*get_the_var_of_env(t_token *node, t_minishell *minishell);
...
void			manage_here_doc(t_token **current, t_minishell *exit_code,
	char *content, int alone);
void			check_line(t_token **lst, t_minishell *minishell,
	t_minishell *exit_code);
...
int				manage_output_redirection(char *node_content, int last_file,
	t_minishell *minishell);
int				manage_input_redirection(t_token **current, char *node_content,
	int first_file, t_minishell *exit_code);
int				manage_append_redirection(char *node_content, int last_file,
	t_minishell *minishell);
int				manage_redirection_input(t_token **current,
	t_minishell *exit_code, int first_file);
int				manage_redirection_output(t_token **current, int last_file,
	t_minishell *minishell);
int				manage_cmd_pipe(t_token **current, t_minishell *exit_code,
	int last_file, t_minishell *minishell);
void			exec_simple_cmd(t_token **current, t_minishell *exit_code,
	t_minishell *minishell);
void 			execute_commands_with_pipes_and_redirections(t_token **lst,
...
int				manage_solo_output_redirection(char *node_content, int last_file,
	t_minishell *minishell);
int				manage_solo_input_redirection(t_token **current, char *node_content,
	int first_file, t_minishell *exit_code);
void			exec_pipe_simple_cmd(t_token **current, t_minishell *exit_code, char **env);
void			exec_cmd_with_pipe(t_token **current, t_minishell *exit_code,
	int last_file, t_minishell *minishell);
int				manage_pipe_output(t_token **current, t_minishell *minishell,
	t_minishell *exit_code);
int				how_many_output(t_token **current);
...
void			remove_quotes_cmd_line(t_token *node, t_minishell *minishell);
...
void    		free_that_final_lst(t_token **token);
...
// REDIRECTION
void			test_redirection_input(t_token *clean_node);
int				redirection_input(t_token *clean_node);
void			redirection_output(t_token *clean_node);
void			redirection_append(t_token *clean_node);
int				input_in_bloc(t_token *token, int i);
void			redirection_here_doc(t_token *clean_node, int i);
...
void			get_var_of_env(t_token *node);
...
void	free_lst_content(t_token **token);
